...
export default logic(null, async (data, session) => {
  ...
  if (!customer){
    ...
    await Promise.all([insert(() => db.create(db.OBond, { ...CUSTOMER?.bondInfo }, session)), insert(() => db.bulkCreate(db.Mrtno, CUSTOMER?.bondInfo?.o_c_bondmrtnos, session)), insert(() => db.bulkCreate(db.Relno, CUSTOMER?.bondInfo?.o_c_bondrelnos, session)), insert(() => db.bulkCreate(db.OCMortgage, CUSTOMER?.mrtInfo, session))]);
  ...
  if (customer){
    ...
    await update({ type : "guarantee", data : CUSTOMER?.guarenteeInfo, where: {
      ...where,
